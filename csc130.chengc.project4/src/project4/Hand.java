...
public class Hand{
	...
	public boolean hasRank(int rank) {
		GoFishCard temp = new GoFishCard(rank);
		return hand.contains(temp);
	}
	/**
	...
	public LinkedList<GoFishCard> findRank(int rank){
		LinkedList<GoFishCard> ranks = hand;
		for (GoFishCard card : ranks) {
	...
	public int evaluate() {
		int books = 0;
		for (int i = 0; i < 13; i++) {
			LinkedList<GoFishCard> discardRank = this.findRank(i);
			if (discardRank.size() == 4) {
				books++;
				hand.removeAll(discardRank);
			}
		}
		return books;
	}
